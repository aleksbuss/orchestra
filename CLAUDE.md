...
> **🚧 OPEN — next-session handoff (updated 2026-06-22).** Current branch **`feat/moa-bypass-no-double-gen`** is **12 commits ahead of `main`, NOT merged, no PR yet** (Sprint 1 bypass double-gen + Sprint 2a/2b MoA aggregator collapse + PM #76 loop-guard & governor anchor + PM #77 proposer no-tools + scripts/e2e typecheck coverage + data-backups exclude). Gates green: `npm run typecheck` (now main + node-side via `tsconfig.node.json`), `npm run lint` (0 errors), full suite **2755 tests**. **Opening a PR for this branch is itself an open item.** Unfinished work, priority order:
>
