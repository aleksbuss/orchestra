...
> **🚧 OPEN — next-session handoff (updated 2026-06-22, post-merge).** Both feature branches are now **MERGED to `main`** (and deleted): **PR #16** (Sprint 1 bypass double-gen + Sprint 2a/2b MoA aggregator collapse + PM #76 loop-guard & governor anchor + PM #77 proposer no-tools + scripts/e2e typecheck coverage + data-backups exclude) and **PR #15** (PM #78 backup `fs.cp` race + PM #79 MoA Router output cap — renumbered from #76/#77 on rebase to resolve the number collision with PR #16). Gates green on `main`: `npm run typecheck` 0 (main + node-side via `tsconfig.node.json`), `npm run lint` 0 errors, full suite **2760 tests**, both CIs (lint/typecheck/tests + Playwright E2E) passed. Unfinished work, priority order:
>
> 1. **Sprint 2c — flip `inlineSynthesis` default ON. MEASUREMENT DONE (2026-06-22); only the flip + doc updates remain.** Live A/B, deepseek-v3 orchestrator + proposers over OpenRouter, N=3 then **N=8** diverse prompts (REST / locking / OOP / iterative-Fibonacci code / DB-index / HTTP1.1-vs-2 / microservices-vs-monolith / CAP). Result: **quality HOLDS** (ON answers equivalent or marginally better, incl. code/long-form/contentious); **latency −31%** (40.6s→27.8s, every prompt faster); **completion tokens −16%**; **cost only −3.8%** (the final stream's large system prompt dominates token count and is shared by BOTH paths — the collapse only removes the aggregator generation, so the real win is latency, not cost). Collapse fired 8/8 ON, aggregator 8/8 OFF; quality recommendation = **flip**. **How to flip:** the gate is `settings.aggregator?.inlineSynthesis === true` in [`moa.ts`](src/lib/agent/moa.ts) (grep it) — either change to `!== false` (default ON unless explicitly disabled) OR add `aggregator: { mode: "synthesis", inlineSynthesis: true }` to `DEFAULT_SETTINGS` in [`settings-store.ts`](src/lib/storage/settings-store.ts) (it has NO `aggregator` field today). **Same PR must also:** reconcile the collapse-gate tests in [`moa.test.ts`](src/lib/agent/moa.test.ts) (the "inlineSynthesis OFF (default)" case encodes the OLD default — a gate-level `!== false` flip makes it FAIL because `fakeSettings()` has no `aggregator`; a `DEFAULT_SETTINGS` flip leaves it green since the test passes `settings` directly and bypasses defaults, so add a default-ON assertion instead), the README "MoA pipeline" prose + mermaid (doc-freshness §7), [`docs/moa-aggregator-collapse.md`](docs/moa-aggregator-collapse.md) §9.1/§10, and this note. Caveat: measured with deepseek proposers (reliable text); gemini-2.5-flash proposers can empty (item 5) — orthogonal to the collapse.
...
> 4. **Embeddings NOT configured → disagreement detection (PM #39) + trace-memory (PM #51) silently OFF in baseline.** `embeddingsModel` = `google/gemini-embedding-001` with NO `apiKey`. For 2c we pulled a LOCAL `nomic-embed-text` via Ollama (768 dims) and verified it works through Orchestra: config `{ provider: "ollama", model: "nomic-embed-text", baseUrl: "", dimensions: 768 }` + `traceMemory.enabled: true`. Settings were RESTORED to baseline afterward (so it's OFF again now; the Ollama model stays pulled). Operator decision: enable local Ollama embeddings OR add a cloud embeddings key to the vault.
