...
function getStatusIcon(status: SwarmNodeStatus) {
  switch (status) {
    case "queued":
      return <Clock className="w-3 h-3 text-amber-600 dark:text-amber-600 dark:text-amber-400" />;
    case "running":
      return <Loader2 className="w-3 h-3 text-blue-600 dark:text-blue-400 animate-spin" />;
    case "completed":
  ...
  return (
    <div className="mx-4 my-2 glass-panel rounded-xl overflow-hidden text-sm">
      ...
      <div
        ...
        className="w-full flex items-center justify-between p-3 px-4 hover:bg-foreground/5 transition-all duration-200 cursor-pointer"
      >
        <div className="flex items-center gap-3">
          ...
          {allCompleted && (
            <button
              ...
              className="ml-4 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-foreground/5 hover:bg-foreground/10 text-muted-foreground text-xs font-medium transition-colors border border-border/70"
