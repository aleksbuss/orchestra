...
Orchestra is a hard-fork of [Eggent](https://github.com/eggent-ai/eggent). From Eggent, Orchestra inherits the **workspace scaffold**: the JSON-on-disk storage model, the projects / memory / knowledge / MCP / cron / Telegram subsystems, the Next.js application shell, and a base single-agent loop. The majority of those imported scaffold files remain substantially intact.

**The Mixture-of-Agents (MoA) ensemble is original to Orchestra — it does not exist in Eggent** (whose `src/lib/agent/` is a four-file single-agent design with no proposers/aggregator/swarm). The MoA layer — parallel proposers, a code-guaranteed Skeptic, dynamic persona generation, tournament aggregation, reflection/disagreement detection, and trace-memory (~15 files under `src/lib/agent/`, incl. `moa.ts` at ~1.6k LOC) — is Orchestra's defining capability. On top of it Orchestra also adds a data-isolation layer (`ORCHESTRA_DATA_DIR`), chat soft-delete + index-integrity recovery, observability/post-mortem tooling, an expanded test suite (113 → 161 test files), a Russian/English bilingual surface, the multi-provider key waterfall, and numerous security/reliability fixes (~30k net lines across 100+ new files). The upstream's MIT license is preserved; see [`NOTICE.md`](../NOTICE.md) for full attribution.
