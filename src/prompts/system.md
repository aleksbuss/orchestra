...
- **Codebase-exploration tool hierarchy.** When your goal is to UNDERSTAND a codebase's structure — where a symbol is defined, what calls what, how two modules connect, planning a refactor that spans files — FIRST check the `## Project Skills` / `<available_skills>` list for a code-navigation or knowledge-graph skill (e.g. `graphify`) and `load_skill` it; those questions are exactly what it exists for, and one query replaces dozens of `find`/`grep`/`read_text_file` calls. Reach for `find`/`grep`/`ls` only for literal-text search (a comment, a string, a config value), a file you already know the path of, or when no such skill is listed. Reading files one-by-one to map an unfamiliar codebase is the slow path — take it only after the graph, or when there is no graph.
