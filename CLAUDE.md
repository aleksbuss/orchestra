...
25. **File-size discipline.** Soft cap 800 lines per `.ts`/`.tsx`; past 1500 the file MUST be decomposed by the next substantive PR. Net growth in an already-bloated module is forbidden — extract something equivalent.
    - **ONE recorded exception: `agent.ts` (1851 LOC).** It crossed 1500 and several substantive PRs landed without decomposing it, so the rule as written has been violated — say so rather than pretending otherwise. The seams are planned in [`file-size-decomposition.md`](docs/references/file-size-decomposition.md). **Its old justification — a cut buys "maintainability nobody is currently spending against" — is WITHDRAWN as false:** 42 commits touched this file in 90 days, 10 in 30. What a cut actually risks is the hottest path in the system, so make it the way 2026-09-12 did — lift ONE read-only stage out of a closure, verbatim, and land the direct tests the closure made impossible — never a facade rewrite, and never the `streamText` call itself.
    - **The zero-net-growth clause does NOT bind this file, and nothing enforces it.** An earlier version of this rule claimed the clause was "what is actually protecting the file". That was false and is withdrawn: the file went 1774 → 1880 → 1977 with no offsetting extraction until 2026-09-13 paid one (−126), and it stayed green because the only gate touching it — [`claude-md-drift.test.ts`](src/claude-md-drift.test.ts) — compares the DOCUMENTED number against the real one with a `max(50, 5%)` tolerance (±93 lines here) and has no growth clause at all. A gate was deliberately NOT built: enforcing a size ceiling on a file already exempted from decomposition is ceremony, and a permanently-red or trivially-waived gate is worse than an honest note. **So this is the honest note.** If you add substantial logic here, you are adding it to an 1851-line file with nothing stopping you — decide that on purpose. Adding a second file to this list is not an option — fix one before recording another.
