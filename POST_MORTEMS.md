...
**Detection:** On-disk forensics + a throwaway LIVE probe of the real model. Decisive: a system prompt that defines a tool but withholds the native channel makes `qwen/qwen3-coder` emit EXACTLY `<tool_call>{"name":"write_text_file","arguments":{…}}</tool_call>` as message text. Orchestra had no parser for that shape — only `unwrapSerializedResponseCall` (PM #61), which by design unwraps ONLY the `response` tool and only a bare-JSON blob (see `unwrap-response.test.ts`: a `search_web`/`write_text_file` JSON call is deliberately passed through). So any non-`response` call printed as markup was persisted verbatim as the assistant message.
