...
⚠️ **`query`'s default budget is waived in the common case.** `--budget N` is documented as "cap output at N tokens (default 2000)". The budget is measured over the whole rendered text and the overflow IS detected — but the computed cut is then discarded whenever no *whole node entry* would be lost, and since nodes render before edges and are short, they normally all fit. The full output then ships, edges included. Measured on a 5100-node graph: one `query` emitted **29,977 chars (~7,500 tokens), 3.7× the stated default**, byte-identical at `--budget 2000` and `--budget 8000`. Only a budget small enough to cut a node (`--budget 500` there) truncated at all. **Always pass an explicit, small `--budget`** — a large one is indistinguishable from the default. And there is a **cliff**: measured on that graph, `--budget 1700` returned 5,490 chars while `--budget 1800` returned **29,976** — asking for 6% more budget produced 5.5× more output, because the last node finally fit and the whole edge block shipped with it. **If output is too big, tune `--budget` DOWN. Never nudge it up to "get a bit more context"** — you may cross the cliff and take the entire graph neighbourhood instead. Natural-language questions also seed the traversal from common English words (a question containing "path" seeded the node literally named `path`, exploding it to 257 nodes, 193 of them cut) — **query with identifiers, not prose**.
