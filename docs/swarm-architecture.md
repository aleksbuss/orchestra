...
1. **Forced Skeptic (PM #37).** After generation, if no persona is classified as a `reviewer` (via `detectProposerRole`), the canonical Adversarial Critic is force-injected (evicting the weakest tail persona if already at the cap). CLAUDE.md's "one role is ALWAYS a QA Auditor / Skeptic" is enforced in code, not just prompt-hoped. **Edge case found by the DoubleTake pass — now FIXED (PM #91).** The injection was gated on `object.requiresSwarm`, so the ONE corner where `forceSwarm` is on AND the Router returned `requiresSwarm: false` skipped the bypass (Force wins) but fanned the personas out **without the guaranteed Skeptic** (the Router never receives `forceSwarm`, so it couldn't know the personas would run). The fix drops the gate: injection is now unconditional (`if (!hasSkeptic)`), which is free on the genuine-bypass path (personas discarded before any proposer runs) and mirrors the catch/fallback branch that already guaranteed a skeptic ungated. Two tests that had *encoded* the gap (asserting the critic was NOT injected on the bypass verdict) were flipped to assert the invariant holds.
...
- **Forced-Skeptic edge case (strongest catch) — FIXED (PM #91).** The skeptic force-injection was gated on `object.requiresSwarm`, so `forceSwarm` over a Router `requiresSwarm: false` ran personas without the guaranteed Skeptic. Verified real (a re-audit walked the `(requiresSwarm, forceSwarm)` truth table, confirmed the state reachable and the fan-out unguarded, and found the existing test suite had encoded the gap). Fixed by making the injection unconditional (`if (!hasSkeptic)`); the two bug-encoding tests were flipped. This was a genuine (narrow) CODE gap, not just a doc gap.
