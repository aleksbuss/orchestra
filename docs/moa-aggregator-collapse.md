...
**Status:** **DONE** — 2a + 2b shipped (PR #16), and **2c flipped the default ON (2026-06-22)**: `DEFAULT_SETTINGS.aggregator.inlineSynthesis = true`, so the collapse is the production default (the `moa.ts` gate stays `=== true`; opt out with `inlineSynthesis: false`). Backed by the N=8 live A/B — quality held, latency −31%, completion tokens −16%, cost −3.8% (see §10). **Only the tournament collapse remains** (deferred micro-sprint, §9.3).
...
- [x] **Flipped `inlineSynthesis` default to ON (2026-06-22).** Chosen mechanism: `DEFAULT_SETTINGS.aggregator = { mode: "synthesis", inlineSynthesis: true }` (settings-store.ts) — NOT a gate change. The `moa.ts` gate stays `=== true`, so production (every `getSettings()` caller) collapses by default, while a unit test passing `fakeSettings()` directly keeps the standalone aggregator unless it opts in (this kept the ~15 aggregator-path tests green — a gate-level `?? true` flip broke them all). Updated: `moa.test.ts` (absent-flag → aggregator, explicit-`false` opt-out) + a DEFAULT_SETTINGS default-ON pin in `settings-store.test.ts`; README MoA prose + a note under the mermaid; §9.1/header above; CLAUDE.md handoff + §1. NOTE: the trace-memory quality-score was a weak proxy (1.000 on both arms) — judged by answers + latency/token deltas.
