...
Same review, second catch: `isFreeModelId` tested only the `:free` SUFFIX, but `openrouter/free` is the free auto-router and carries no suffix — and it is a live value in this operator's config (`utilityModel`). The notice would have called it "stronger" and told them to turn Free Mode off, landing on another free model: the precise failure `e411029` exists to remove. Now `/:free$/ || /(^|\/)free$/`.
