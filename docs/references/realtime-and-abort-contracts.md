...
**PM #121 — the same exclusivity has a corollary the other direction: don't re-check the signal downstream of the callback it already gated.** Reaching `onError`'s (or `onFinish`'s) body at all already proves the SDK evaluated `abortSignal.aborted` and dispatched AWAY from `onAbort` — so any code called only from inside those callbacks (the recovery/failover ladder, `recoverPrimaryStreamFailure`, `generateFinalAnswerWithFailover`) that later checks the SAME `abortSignal` for "was this a user cancel" is asking a question the SDK already answered no to. Live-confirmed: `req.signal` flipped `aborted: true` ~42ms into a recovery attempt with zero user action, most likely the primary stream's own error teardown touching the same signal object. Both call sites (`agent.ts`'s `onError`, `agent-response.ts`'s no-answer-delivered branch) now pass `abortSignal: undefined` into the recovery ladder — never `options.abortSignal` / the caller's raw signal — guarded by [`recovery-abortsignal-contract.test.ts`](../../src/lib/agent/recovery-abortsignal-contract.test.ts). `undefined` does not make those calls unbounded: `callDeadlineSignal(undefined)` still returns a plain `AbortSignal.timeout(...)`.

**Non-streaming calls use `callDeadlineSignal()`, not the watchdog.** `generateText`/`generateObject` emit no chunks, so there is nothing to watch; the instrument there is a plain total-duration deadline (`AbortSignal.any([caller, AbortSignal.timeout(N)])`, `ORCHESTRA_CALL_DEADLINE_MS`, default 240s). That pattern lived in `moa-proposers.ts` for months and was applied at 2 of 9 callsites — including neither the Router, nor the aggregator, nor the delivery ladder, nor cron/Telegram. [`call-deadline-contract.test.ts`](../../src/lib/agent/call-deadline-contract.test.ts) now counts them, because nothing was counting and that is why it drifted. It also asserts the STREAMING call uses the watchdog and NOT a deadline — a wall-clock cap there would kill a legitimately long agentic turn.
