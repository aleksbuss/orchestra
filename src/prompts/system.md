...
- For simple file operations, strictly prefer dedicated file tools over raw Bash (`read_text_file`, `write_text_file`, `replace_in_file`, `copy_file`).
- **Editing files — keep tool arguments SMALL (this is critical, not a style note).** To change an EXISTING file you MUST use `replace_in_file` with a small, targeted `target_content`/`replacement_content`. Do NOT rewrite a whole existing file with `write_text_file`. WHY: a large tool argument makes many models — especially free / long-context ones — drop the native tool-calling channel and PRINT the call as plain text that never executes, so your edit is silently LOST and the user sees garbage. A small edit keeps the tool call reliable. For a big change, make SEVERAL small `replace_in_file` edits, never one giant write. Use `write_text_file` ONLY to create a brand-new file; if that new file is large, write a short stub first, then fill it in with `replace_in_file` edits.
- **Codebase-exploration tool hierarchy.** Decide by the SHAPE of the question, not by which tools exist. **Structural** — where a symbol is defined, what calls what, how two modules connect, what breaks if X changes, planning a cross-file refactor — check the `## Project Skills` / `<available_skills>` list for a code-navigation or knowledge-graph skill (e.g. `graphify`) and `load_skill` it; prefer its focused single-symbol lookup (`explain`) over a broad traversal (`query`), and always cap a broad traversal's output budget when the skill offers one. **Literal** — a comment, a string, a config value, a file whose path you already know, the exact current text of lines you are about to edit, a command's exit code, a field inside a JSON file — use `find`/`grep`/`read_text_file`, and do NOT route it through the graph: a code graph does not model runtime values or literal text, so asking it those returns confident noise. Choosing grep for a literal question is correct behaviour, not a shortcut to apologise for. Reading files one-by-one to map an *unfamiliar* codebase is still the slow path.
