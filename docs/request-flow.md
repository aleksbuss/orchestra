...
4. **Filter + branch** (`isSuccessfulDraft`, then `isPrintedMarkupDraft` — a draft that PRINTED a tool call as text is dropped here so it reaches neither a delivery path nor a prompt, PM #134): **0** deliverable → `degradedToSingleAgent` (surfaced to the operator; an honest degradation notice when the drafts were markup rather than failures); **1** → skip aggregation; **≥2** → aggregate.
