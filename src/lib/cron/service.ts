...
/**
 * Wrap `computeJobNextRunAtMs` with the `state.unresolvable` cache. Every
 * callsite that previously called `computeJobNextRunAtMs` directly should
 * go through this — both the side effect (cache set) and the read (short-
 * circuit return) live here.
 *
 * Why: a cron schedule like `0 0 30 2 *` (Feb 30 — never) exhausts the
 * 2-year lookahead in `computeNextRunAtMs` (~1M iterations, ~28s wall-clock
 * on a typical laptop). Without this cache, the scheduler tick would
 * re-run that loop every minute for every such job — a DoS vector for
 * any authenticated user who can submit cron jobs. The flag is sticky
 * until a `patch.schedule` clears it via `applyPatch`.
 */
function computeNextRunWithCache(job: CronJob, nowMs: number): number | undefined {
  if (job.state.unresolvable && job.schedule.kind === "cron") {
    return undefined;
  }
  const next = computeJobNextRunAtMs(job, nowMs);
  if (next === undefined && job.schedule.kind === "cron" && job.enabled) {
    job.state.unresolvable = true;
  }
  return next;
}
...
function applyPatch(job: CronJob, patch: CronJobPatch, nowMs: number): void {
  ...
  job.state.nextRunAtMs = computeNextRunWithCache(job, nowMs);
}
...
async function finalizeJobRun(projectIdRaw: string, jobId: string, result: RunResult): Promise<void> {
  ...
  await withProjectStore(projectId, async ({ store, markChanged }) => {
    ...
    if (job.schedule.kind === "at") {
      ...
      // Use the cached wrapper — if the schedule is unresolvable (already
      // known impossible), we must NOT keep scheduling backoff retries that
      // re-fire the same impossible expression. naturalNext === undefined
      // for unresolvable crons → nextRunAtMs stays undefined → no rescheduling.
      const naturalNext = computeNextRunWithCache(job, result.endedAt);
      const backoffNext = result.endedAt + backoffMs;
      job.state.nextRunAtMs = job.state.unresolvable
        ? undefined
      ...
      const naturalNext = computeNextRunWithCache(job, result.endedAt);
      if (job.state.unresolvable) {
        // Impossible cron expression — do not reschedule.
        job.state.nextRunAtMs = undefined;
