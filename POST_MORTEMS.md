...
**Rule:** Tool-capability availability must be a function of **trust context**, not just a global on/off. Any tool that executes code or touches the host beyond the `assertPathInside` sandbox (shell, package install, process spawn) must be **denied to untrusted (external-user-reachable) triggers by default**, with an explicit, documented opt-in. And the trust flag MUST be propagated to every delegated/subordinate run — a security gate bypassable by one hop of delegation is theater. This is the tool-capability analog of PM #58's "enforce the control at EVERY entry point, and a new entry inherits ZERO guards." When adding a tool, ask: "what happens if an untrusted Telegram user reaches this via prompt injection?"
