...
| A chat is stuck, a chat vanished from the sidebar, reconstructing a run | [`docs/references/observability-runbook.md`](docs/references/observability-runbook.md) | `/api/debug/chat/<id>` one-shot, manual fallback steps, chat-index integrity |
...
⚠️ **`query` is the expensive one, and its default budget is waived in the common case.** `--budget N` is documented as "cap output at N tokens (default 2000)". Root cause read from the installed source (`graphify/serve.py:1093`, v0.9.44): the budget IS measured over the whole rendered text, the overflow IS detected and a cut point IS computed — then **`if cut_count == 0: return output` discards the cut** whenever no *whole NODE line* would be lost. Nodes render before edges and are short, so they normally all fit and `cut_count` is 0; the full output ships, edges included. `query "forcedAnswer"` emits **29,977 chars (~7,500 tokens), 3.7× over the stated default**, byte-identical at `--budget 2000` and `--budget 8000` (both take that same early return). Only a budget small enough to cut a node — `--budget 500` here — truncates at all. **So: never call `query` without an explicit, small `--budget`, and tune it DOWN, never up** — measured, `--budget 1700` → 5,490 chars but `--budget 1800` → **29,976**: +6% budget, 5.5× output, because the last node fit and the whole edge block shipped with it. A natural-language question also seeds the traversal from common English words (asking about a "path" seeded the node literally named `path`, exploding it to 257 nodes) — prefer identifiers.
...
- **Verify on a REAL production build before calling anything done:** `npm run build && npm run start`, then exercise the feature. **Nothing else in this repo builds** — vitest doesn't, and Playwright starts `next dev` — so an entire class of defect is invisible to every gate. Two P0/P1 bugs were found exactly this way on 2026-08-25 (PM #110, PM #111), and a "fix" that passed `verify:strict` and e2e broke every background turn in production. Minimum sweep: log in, open `/dashboard/cron`, send one interactive turn and one `background: true` turn. ⚠️ `npx playwright test` overwrites `.next` with a dev build — rebuild before starting.
...
- **Check Free Mode's cold-boot ids:** `npm run verify:free-fallbacks` — hits the live OpenRouter catalogue and fails if any id in `FREE_ROUTER_FALLBACKS` / `FREE_GENERAL_FALLBACKS` has been withdrawn or lost the capability its list promises. Deliberately NOT in CI (network + third-party churn); run it whenever you touch either list. The offline half of the contract is pinned in `free-mode.test.ts`.
...
12b. **Auth decisions come from an allowlist of paths, never from a pattern in the path.** In `middleware.ts`, application-route exclusions (`/api/`, `/dashboard`) run BEFORE any "looks like a static asset" test — the attacker picks the spelling, and `%2f` is not a literal `/`. A dotted segment (`/api/debug/chat/foo.json`) skipped every auth check and read chat content anonymously (PM #110).
...
15b. **A generation is not a delivery — JUDGE the output before recording a model success or ending a failover ladder.** A tool-less path that accepts any non-empty string accepts printed tool markup, stops the cascade at the endpoint that just failed, and heals its breaker on the way out (PM #134). Gate with `gateForcedAnswer` (`src/lib/agent/printed-tool-call.ts` — the leaf module both `agent-response.ts` and the ladder import; never import the gate back out of `agent-response.ts`).
...
25. **File-size discipline.** Soft cap 800 lines per `.ts`/`.tsx`; past 1500 the file MUST be decomposed by the next substantive PR. Net growth in an already-bloated module is forbidden — extract something equivalent.
    - **ONE recorded exception: `agent.ts` (1863 LOC).** It crossed 1500 and several substantive PRs landed without decomposing it, so the rule as written has been violated — say so rather than pretending otherwise. The seams are planned in [`file-size-decomposition.md`](docs/references/file-size-decomposition.md). **Its old justification — a cut buys "maintainability nobody is currently spending against" — is WITHDRAWN as false:** 42 commits touched this file in 90 days, 10 in 30. What a cut actually risks is the hottest path in the system, so make it the way 2026-09-12 did — lift ONE read-only stage out of a closure, verbatim, and land the direct tests the closure made impossible — never a facade rewrite, and never the `streamText` call itself.
    - **The zero-net-growth clause does NOT bind this file, and nothing enforces it.** An earlier version of this rule claimed the clause was "what is actually protecting the file". That was false and is withdrawn: the file went 1774 → 1880 → 1977 with no offsetting extraction until 2026-09-13 paid one (−126), and it stayed green because the only gate touching it — [`claude-md-drift.test.ts`](src/claude-md-drift.test.ts) — compares the DOCUMENTED number against the real one with a `max(50, 5%)` tolerance (±94 lines here) and has no growth clause at all. A gate was deliberately NOT built: enforcing a size ceiling on a file already exempted from decomposition is ceremony, and a permanently-red or trivially-waived gate is worse than an honest note. **So this is the honest note.** If you add substantial logic here, you are adding it to an 1863-line file with nothing stopping you — decide that on purpose. Adding a second file to this list is not an option — fix one before recording another.
26. **Before deleting or overwriting anything in `data/`, copy it aside.** There is no undo.
27. **A test that writes through a storage module sets `ORCHESTRA_DATA_DIR` BEFORE importing it** — 17 modules capture `getDataDir()` at load, and ESM hoists static imports above every hook, so a `beforeAll` redirect is too late (import the store lazily). Assert the redirect resolved; it fails silently otherwise. Prove it suite-wide with `touch marker && npm test && find data -newer marker` — grep undercounts (PM #100). **And the counterpart obligation, on the WRITER:** a module that newly resolves a path under `data/` must make the SAFE destination its DEFAULT under `VITEST`/`NODE_ENV=test` (honouring an explicit `ORCHESTRA_DATA_DIR`, and quarantining only the destination so the tests still exercise real I/O). This rule alone cannot protect you — it addresses test authors, and a new writer silently enrolls every EXISTING test, which cannot have complied. Adding `dataPath()` to `model-health.ts` enrolled 68 of them and `npm test` deleted the live snapshot (PM #136). Never put an `unlink` inside a helper whose name says "reset".
...
| [`workdir-resolution-contract.test.ts`](src/lib/storage/workdir-resolution-contract.test.ts) | The retired `getWorkDir`/`resolveWorkDirForProject`/`getProjectWorkDir` never return, and only `tool-paths.ts` resolves a project root inside `src/lib/tools` — one resolver, so the path the agent is TOLD can't diverge from the one its tools ACT in (PM #105) |
| [`import-cycle-contract.test.ts`](src/import-cycle-contract.test.ts) | No runtime import cycle under `src/lib` + `src/app` — a cycle is invisible in `next dev` and makes a module's exports `undefined` in a production chunk (PM #111) |
| [`chat-id-guard-contract.test.ts`](src/app/api/chat-id-guard-contract.test.ts) | Every route passing a user-supplied chat id to the store validates it at the ROUTE layer, so a bad id is a 400 and not an unhandled 500 |
