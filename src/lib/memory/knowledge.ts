...
export async function importKnowledgeFile(
  ...
  // PM #21 — defense-in-depth. The route layer is the canonical sanitizer
  // (sanitizeKnowledgeFilename + assertPathInside), but a future caller
  // forgetting to sanitize must not be able to open a traversal primitive
  // here. `assertPathInside` rejects any resolved path outside knowledgeDir.
  let filePath: string;
  try {
    filePath = assertPathInside(knowledgeDir, filename);
  } catch {
    result.errors.push(`Refusing to import "${filename}": path escapes knowledge directory.`);
    return result;
  }
