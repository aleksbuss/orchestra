---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Orchestra deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, orchestra cron scheduling for periodic checks, or version status checks on a machine running orchestra (laptop, workstation, Pi, VPS).
---

# orchestra Host Hardening

## Overview

Assess and harden the host running orchestra, then align it to a user-defined risk tolerance without breaking access. Use orchestra security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim orchestra changes the host firewall, SSH, or OS updates; it does not.
...
5. orchestra gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if orchestra can access sensitive data.
...
### 2) Run orchestra security audits (read-only)

As part of the default read-only checks, run `orchestra security audit --deep`. Only offer alternatives if the user requests them:

1. `orchestra security audit` (faster, non-probing)
2. `orchestra security audit --json` (structured output)

Offer to apply orchestra safe defaults (numbered):

1. `orchestra security audit --fix`

Be explicit that `--fix` only tightens orchestra defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check orchestra version/update status (read-only)

As part of the default read-only checks, run `orchestra update status`.
...
- Credential hygiene notes (location of orchestra creds, prefer disk encryption)
...
- orchestra security audit (re-run)
...
After orchestra install or first hardening pass, run at least one baseline audit and version check:

- `orchestra security audit`
- `orchestra security audit --deep`
- `orchestra update status`

Ongoing monitoring is recommended. Use the orchestra cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `orchestra cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `orchestra update status`
...
Before creating, `orchestra cron list` and match on exact `name`. If found, `orchestra cron edit <id> ...`.
If not found, `orchestra cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `orchestra update status` (preferred for source checkouts and channels)
2. `npm view orchestra version` (published npm version)

## orchestra command accuracy

Use only supported commands and flags:

- `orchestra security audit [--deep] [--fix] [--json]`
- `orchestra status` / `orchestra status --deep`
- `orchestra health --json`
- `orchestra update status`
- `orchestra cron add|list|runs|run`

Do not invent CLI flags or imply Orchestra enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Orchestra compaction:
