...
**Resolution:** Two sprints, both RE-PROMPT rather than blind-execute the scraped markup (parsed args may be mangled — see PM #80 — and re-issuing keeps the SDK validation + loop-guard in the path). **Sprint 1 (detect + safe fallback):** `extractHallucinatedToolCall` ([`agent-response.ts`](src/lib/agent/agent-response.ts)) normalizes every shape (conservative — the markup must DOMINATE the whole message via `^…$` + an args-container requirement, so prose quoting `<tool_call>` never false-matches); `unwrapSerializedResponseCall` now also unwraps a markup-wrapped `response` to prose; `turnHasDeliverableAnswer` returns false for an action-tool hallucination so `resolveTurnContinuation` forces a clean prose answer; `system.md` hard_constraint #5 forbids printing tool-call markup. **Sprint 2 (active self-heal):** `stripHallucinatedTrailingText` drops the markup message so the user never sees XML; `detectActionHallucination` + `attemptToolReissue` ([`agent-tool-reissue.ts`](src/lib/agent/agent-tool-reissue.ts)) run ONE bounded tool-capable generation with a deterministic `REISSUE_CORRECTION` so the model re-issues the call NATIVELY and the action runs — bounded by a chat-scoped retry budget (`recordReissueAttempt`, `REISSUE_MAX_RETRIES=2`, in-memory, evaporates on restart, reset on success). The re-issue's real messages are persisted and billed (`foldTurnUsage` gains `reissueUsage`).
