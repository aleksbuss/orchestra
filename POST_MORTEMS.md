...
**Rule:** a passive injection sourced from a growing store must be BOUNDED at the injection site (per-item + total), independent of any item-COUNT cap — an unbounded item size defeats a count cap. And the token governor must budget for `system + messages`, not messages alone — the system prompt is context too. Both caps are currently a FIXED char/token budget (calibrated for a ~120K reliable window); on a big-window model they under-use the window (safe, not a correctness break) — window-relative budgets are the cross-model refinement.
